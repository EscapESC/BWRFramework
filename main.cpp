...
int main(int argc, char const *argv[])
{
    ...
    while (true)
    {
        if(unit->update() == 0){
            ...
            if ((unit->reactor->reactorPeriod > 40 || unit->reactor->reactorPeriod < 0) && unit->reactor->reactor_power < 20){
            ...
            if (unit->reactor->reactorPeriod < 30 && unit->reactor->reactorPeriod > 0){
                for(int x = 0; x < unit->reactor->SIZE; x++){
                    for(int y = 0; y < unit->reactor->SIZE; y++){
                        unit->reactor->moveRod(x,y,rodPosition);
                    }
